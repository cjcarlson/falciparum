...
all %>% select(run, scenario, OBJECTID, year, Pred) %>% 
  filter(str_detect(run, 'rcp26'), year %in% c(2015, 2100)) %>% 
...
slices %>% pivot_wider(names_from = year, values_from = Pred) %>% 
  mutate(diff = (`2100` - `2015`)) -> slices.runs
...
ggplot(sfcont) + 
  ...
  theme(plot.title = element_text(size = 25))  -> map.rcp26.nolegend
...
all %>% select(run, scenario, OBJECTID, year, Pred) %>% 
  filter(str_detect(run, 'rcp85'), year %in% c(2015, 2100)) %>% 
  group_by(run, scenario, OBJECTID, year) %>% summarize(Pred = mean(Pred, na.rm = TRUE)) -> slices
...
map.rcp26.nolegend + map.rcp85.nolegend + legend + plot_layout(widths = c(4, 4, 2)) -> top
