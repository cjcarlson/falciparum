...
smallfuture %<>% 
  ...
  select(gcm, line, Pred) %>%
  dplyr::rename(Pred2015 = Pred) %>%
  right_join(smallfuture) %>%
  mutate(Pred = (Pred - Pred2015)) %>%
  select(-Pred2015) 

aligner <- smallpast %>% 
  filter(year == 2014, line == 'hist') %>%
  select(gcm, Pred) %>%
  dplyr::rename(Pred2014 = Pred)

smallfuture %<>%
  left_join(aligner) %>%
  mutate(Pred = Pred + Pred2014) %>%
  select(-Pred2014) 
...
# small %>%
#   mutate(line = factor(line, levels = c('nat', 'hist', 'rcp26', 'rcp45', 'rcp85'))) %>%
#   ggplot(aes(y = Pred, group = line, color = line)) +
#   geom_boxplot() + 
#   scale_color_manual(values = c("grey50", "#287DAB", "#4d5f8e", "#C582B2", "#325756")) + 
