...
all_columns <- paste0(
  "mean.diff.",
  rep(years, length(scenarios)),
  rep(substr(scenarios, 4, 6), each = length(years))
)
global_limits <- range(
  sapply(all_columns, function(col) cont[[col]]),
  na.rm = TRUE
)
...
plots[[1]] <- plots[[1]] + labs(tag = "SSP1-RCP2.6", title = "2048-2052")

plots[[2]] <- plots[[2]] + labs(title = "2096-2100")

plots[[3]] <- plots[[3]] + labs(tag = "SSP2-RCP4.5")

plots[[5]] <- plots[[5]] + labs(tag = "SSP5-RCP8.5")
...
map_grid <- ((plots[[1]] + plots[[2]]) /
  ...
  theme(
    plot.tag = element_text(size = 16, angle = 90, hjust = 0.5, vjust = 0.5),
...
ggplot2::ggsave(
  filename = paste0("ED_Figure_projection_maps.", fig_file_type),
  plot = map_grid,
