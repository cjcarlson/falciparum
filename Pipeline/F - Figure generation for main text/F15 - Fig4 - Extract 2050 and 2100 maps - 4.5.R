...
meta %<>% tidyr::extract(run, 
               ...
               remove = FALSE)
rows <- which((meta$RCP %in% c('rcp45')) & (meta$year %in% c(2015:2019, 2048:2052, 2096:2100)))
meta <- meta[rows,]
# Yes I know the below solution is a little janky, god forgive me 
meta$year[meta$year %in% c(2015:2019)] <- 2015
meta$year[meta$year %in% c(2048:2052)] <- 2050
...
vroom::vroom_write(iter.df, "~/Github/falciparum/TempFiles/Fig4BigRCP45.csv")
