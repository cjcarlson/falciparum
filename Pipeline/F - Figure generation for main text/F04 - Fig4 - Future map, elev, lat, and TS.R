...
iter.df <- file.path(
  fut_sum_dir,
  "future_pred_sum_scen_mod_yr_obj.feather"
) |>
  arrow::read_feather() |>
  dplyr::mutate(
...
df <- slice.map2 |>
  left_join(
    elev |> select(OBJECTID, elevmn) |> mutate(OBJECTID = as.factor(OBJECTID))
  ) |>
...
data.to.graph <- file.path(
  fut_sum_dir,
  "future_pred_sum_scen_mod_yr_reg.feather"
) |>
  arrow::read_feather() |>
  ...
  dplyr::mutate(
    ...
    lower = pmax(lower, -4.3), upper = pmin(upper, 2.1)
  ) |>
  dplyr::filter(year > 2015)

# data.to.graph <- here::here("TempFiles", "Fig4Regionals.csv") |>
#   readr::read_csv(show_col_types = FALSE) |>
#   mutate(region = dplyr::recode(region, !!!region_names)) |>
