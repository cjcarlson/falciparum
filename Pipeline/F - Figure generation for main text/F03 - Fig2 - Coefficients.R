...
main <- as.data.frame(t(as.matrix(coefs))) |>
  dplyr::rename(
    "Intvn. 1955-69" = "I(intervention)1",
    "Intvn. 2000-15" = "I(intervention)2"
  )
...
# files = list.files(model_boot_dir)
# stop = length(files) - 1
# files = files[1:stop]

# for (f in 1:length(files)) {
#   tmp = readRDS(file.path(model_boot_dir, files[f]))
...
# main_line <- plotData %>%
#   dplyr::filter(model == "main") %>%
#   dplyr::select(x, response)

# percentile_data <- main_line %>%
#   dplyr::left_join(vcov_boot, by = "x") %>%
#   dplyr::mutate(
#     p05 = response - 1.96 * se,
#     p95 = response + 1.96 * se
#   )
...
inter = df %>%
  ...
  dplyr::mutate(
    var = stringr::str_replace_all(var, "I\\(intervention\\)1", "Int. 1"),
    var = stringr::str_replace_all(var, "I\\(intervention\\)2", "Int. 2")
  )
