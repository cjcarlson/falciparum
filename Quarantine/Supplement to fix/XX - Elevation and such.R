...
latlon %>% as_tibble() %>% select(OBJECTID, mean.diff, lat, lon) %>%
  left_join(elev %>% mutate(OBJECTID = as.character(OBJECTID))) %>% 
  distinct() -> data

data %>%
  left_join(all %>% 
              filter(year %in% c(1900:1930)) %>% 
              group_by(OBJECTID, Region) %>%
              summarize(temp = mean(temp)) %>%
              mutate(OBJECTID = as.character(OBJECTID))) -> data

data %<>% remove_missing()

# data %>% 
#   ggplot(aes(x = lat, y = mean.diff, color = Region)) + geom_point() + theme_bw() +
#   geom_hline(yintercept = 0, col = 'red') + 
#   geom_smooth(method = 'gam', se = F) + 
#   xlab('Latitude') + ylab('Change in prevalence (1900-2014)') -> g1 
# 
# data %>% 
#   ggplot(aes(x = elevmn, y = mean.diff, color = Region)) + geom_point() + theme_bw() +
#   geom_hline(yintercept = 0, col = 'red') + 
#   geom_smooth(method = 'gam', se = F) + 
#   xlab('Elevation (m)') + ylab('Change in prevalence (1900-2014)') -> g2
# 
# data %>% 
#   ggplot(aes(x = temp, y = mean.diff, color = Region)) + geom_point() + theme_bw() +
#   geom_hline(yintercept = 0, col = 'red') + 
#   geom_smooth(method = 'gam', se = F) + 
#   xlab('Mean temperature (1900-1930)') + ylab('Change in prevalence (1900-2014)') -> g3
# 
# library(patchwork)
# 
# g1/g2/g3 +
...
code = c('Sub-Saharan Africa (Central)' = 'Central',
         'Sub-Saharan Africa (East)' = 'East',
         'Sub-Saharan Africa (West)' = 'West',
         'Sub-Saharan Africa (Southern)' = 'Southern')

data %<>%
  mutate(Region = recode(Region, !!!code))
  
data %>% 
  ggplot(aes(x = lat, y = mean.diff)) + 
  geom_hline(yintercept = 0, col = 'red') +   
  geom_point(aes(color = Region)) + theme_bw() +
  geom_smooth(method = 'loess', se = F, col = 'grey50', lwd = 1.3) + 
  xlab('Latitude') + ylab('Change in prevalence (1900-2014)') + 
  scale_color_manual(values = met.brewer("Cross")[c(2,5,6,7)]) -> g1 

data %>% 
  ggplot(aes(x = elevmn, y = mean.diff)) + 
  geom_hline(yintercept = 0, col = 'red') + 
  geom_point(aes(color = Region)) + theme_bw() +
  geom_smooth(method = 'loess', se = F, col = 'grey50', lwd = 1.3) + 
  xlab('Elevation (m)') + ylab('Change in prevalence (1900-2014)') + 
  scale_color_manual(values = met.brewer("Cross")[c(2,5,6,7)]) -> g2

data %>% 
  ggplot(aes(x = temp, y = mean.diff)) + 
  geom_hline(yintercept = 0, col = 'red') + 
  geom_point(aes(color = Region)) + theme_bw() +
  geom_smooth(method = 'loess', se = F, col = 'grey50', lwd = 1.3) + 
  xlab('Mean temperature (1900-1930)') + ylab('Change in prevalence (1900-2014)') + 
  scale_color_manual(values = met.brewer("Cross")[c(2,5,6,7)]) -> g3
...
data %>% 
  ggplot(aes(x = elevmn, y = lat, color = temp)) + 
  geom_point(size = 2.5, pch = 20) + 
  scale_color_gradientn(colors = met.brewer("Morgenstern", n = 11)[1:11], limits = c(18, 28)) + 
  facet_wrap(~ Region) + 
  theme_bw() + ylab("Latitude") + xlab("Elevation (m)") + labs(color = 'Temp.')
